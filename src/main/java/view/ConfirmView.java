 ...
 */
public final class ConfirmView extends JPanel {
    private static final String MOCK_FIGHTER_NAME = "The Prospect";
    private static final String MOCK_WEIGHT_CLASS = "LIGHTWEIGHT";
    private static final String MOCK_OVERALL = "94.2";

    private static final String[] ATTRIBUTE_NAMES = {"STRIKING", "DEFENSE", "TAKEDOWNS", "HEIGHT", "REACH", "CARDIO"};
    private static final String[] ATTRIBUTE_SOURCES = {"Max Holloway", "Islam Makhachev", "Georges St-Pierre", "Kamaru Usman", "Merab Dvalishvili", "Jon Jones"};

    private static final int[] ATTRIBUTE_VALUES = {91, 97, 94, 93, 96, 92};
    public ConfirmView(Runnable backAction, Runnable continueAction) {
        ...
        final JButton spin = UfcTheme.primaryButton("Spin Weight Class");
        final JButton confirm = UfcTheme.primaryButton("Confirm Fighter");

        back.addActionListener(event -> backAction.run());
        confirm.addActionListener(event -> continueAction.run());
    ...
    private JPanel createFighterPanel() {
        ...
        final JTextField name = UfcTheme.textField(24);
        name.setText(MOCK_FIGHTER_NAME);
        namePanel.add(name);
        ...
        for (int index = 0; index < ATTRIBUTE_NAMES.length; index++) {
            ...
            row.add(UfcTheme.body(
                    ATTRIBUTE_VALUES[index] + "  •  "
                            + ATTRIBUTE_SOURCES[index]), BorderLayout.EAST);
            list.add(row);
        }
    ...
    private JPanel createDivisionPanel() {
        ...
        final JLabel division = UfcTheme.centeredLabel(
                MOCK_WEIGHT_CLASS,
                new Font(Font.SANS_SERIF, Font.BOLD, 30), UfcTheme.TEXT);
        final JLabel overall = UfcTheme.centeredLabel(
                MOCK_OVERALL,
                new Font(Font.SANS_SERIF, Font.BOLD, 58), UfcTheme.ACCENT);
        ...
        panel.add(UfcTheme.centeredLabel(
                "WEIGHTED OVERALL", UfcTheme.BODY_BOLD, UfcTheme.MUTED));
        panel.add(overall);
        panel.add(Box.createVerticalStrut(20));
        panel.add(UfcTheme.centeredLabel(
                "Placeholder values only", UfcTheme.SMALL, UfcTheme.MUTED));
        panel.add(Box.createVerticalGlue());
        return panel;
    }
}